...
const TERRAIN_DRAWING_STRIDE = 4;
...
const localTerrainUrl = (fileName: string) =>
    `${import.meta.env.BASE_URL.replace(/\/?$/, "/")}data/measuring/${fileName}`;

export const loadTerrainGrid = _.memoize(async (): Promise<TerrainGrid> => {
    const [metadataResponse, valuesResponse] = await Promise.all([
        fetch(localTerrainUrl("terrain-50.json")),
        fetch(localTerrainUrl("terrain-50.bin")),
    ]);
    if (!metadataResponse.ok || !valuesResponse.ok) {
        throw new Error("Could not load static OS Terrain 50 data");
    }
    const metadata = (await metadataResponse.json()) as TerrainMetadata;
    const values = new Int16Array(await valuesResponse.arrayBuffer());
    if (values.length !== metadata.width * metadata.height) {
        throw new Error("Static OS Terrain 50 data has an unexpected size");
    }
    return { metadata, values };
});

const rawTerrainValue = (terrain: TerrainGrid, column: number, row: number) => {
    const { metadata, values } = terrain;
    if (
        column < 0 ||
...
export const terrainElevationMeters = (
    terrain: TerrainGrid,
    latitude: number,
    longitude: number,
) => {
    const { metadata } = terrain;
    ...
    const neighbours = [
        {
            value: rawTerrainValue(terrain, left, top),
            weight: (1 - horizontalFraction) * (1 - verticalFraction),
        },
        {
            value: rawTerrainValue(terrain, left + 1, top),
            weight: horizontalFraction * (1 - verticalFraction),
        },
        {
            value: rawTerrainValue(terrain, left, top + 1),
            weight: (1 - horizontalFraction) * verticalFraction,
        },
        {
            value: rawTerrainValue(terrain, left + 1, top + 1),
...
const averageBlockElevation = (
    ...
    let valueCount = 0;
    for (let rowOffset = 0; rowOffset < TERRAIN_DRAWING_STRIDE; rowOffset++) {
        for (
            let columnOffset = 0;
            columnOffset < TERRAIN_DRAWING_STRIDE;
            columnOffset++
        ) {
            const elevation = rawTerrainValue(
                terrain,
                startColumn + columnOffset,
                startRow + rowOffset,
            );
            if (elevation === null) continue;
            elevationTotal += Math.abs(elevation);
            valueCount++;
        }
    }
    return valueCount === 0 ? null : elevationTotal / valueCount;
};

export const terrainCloserToSeaLevelPolygon = (
    terrain: TerrainGrid,
    bbox: [number, number, number, number],
    ...
    const columnCount =
        Math.floor((lastColumn - firstColumn) / TERRAIN_DRAWING_STRIDE) + 1;
    const rowCount =
        Math.floor((lastRow - firstRow) / TERRAIN_DRAWING_STRIDE) + 1;
    if (columnCount < 2 || rowCount < 2) return null;

    const threshold = Math.abs(referenceElevationMeters);
    const contourValues = new Float32Array(columnCount * rowCount);
    for (let row = 0; row < rowCount; row++) {
        for (let column = 0; column < columnCount; column++) {
            const elevation = averageBlockElevation(
                terrain,
                firstColumn + column * TERRAIN_DRAWING_STRIDE,
                firstRow + row * TERRAIN_DRAWING_STRIDE,
            );
    ...
    const blockCenterOffset = (TERRAIN_DRAWING_STRIDE - 1) / 2;
    const coordinates = contour.coordinates.map((polygon) =>
        polygon.map((ring) =>
            ring.map(([column, row]) => {
                const globalColumn =
                    firstColumn +
                    (column - 0.5) * TERRAIN_DRAWING_STRIDE +
                    blockCenterOffset;
                const globalRow =
                    firstRow +
                    (row - 0.5) * TERRAIN_DRAWING_STRIDE +
                    blockCenterOffset;
